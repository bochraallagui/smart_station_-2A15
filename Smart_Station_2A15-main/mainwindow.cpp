    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->le_nbticket->setValidator( new QIntValidator(0, 9999999, this));
...
void MainWindow::on_tabWidget_3_currentChanged(int index)
{
  ...
  // axe des ordonnées
                  ui->plot_3->yAxis->setRange(0,15000);
...
void MainWindow::on_pb_ajouter_trajet_clicked()
{
    ...
    int idtrajet=ui->lineEdit_id->text().toInt();
...
else
{
    ...
    trajet t(idtrajet,depart,redandance,terminus,code,date);


        bool test1=t.ajouter();
        //id=ui->tableView->selectionBehavior();
       // bool test2=S.supprimer(idtrajet);
...
void MainWindow::on_supprimer_clicked()
{
    ...
    //int idtrajet =ui->lineEdit_2->text().toInt ;
      int idtrajet=ui->lineEdit_2->text().toInt();
    bool test =Etmp->supprimer(idtrajet);
    if (test)
    {     ui->tableView_2->setModel(Etmp->afficher());
...
{
    trajet T ;
     int idtrajet=ui->lineEdit_id_2->text().toInt();
   ...
   int red=ui->lineEdit_red->text().toInt();
    ...
    {
    trajet T1(idtrajet,depart,red,ter,code,dat);
    //bool test= S.modifier(id,nom,prenom,fonction,date_naissance);
     bool test;
     test= T1.modifier( idtrajet , depart, red, ter,code,dat);
    if (test)
        //[☺]
       {
...
void MainWindow::on_tableView_2_activated(const QModelIndex &index)
{
    ...
    QSqlQuery qry ;
     ...
     qry.prepare("SELECT prix, depart , idtrajet , duree , terminus, date_t FROM trajet");
    if (qry.exec())
    {
...
{
    int choix=0;
    if(ui->comboBox_2->currentText()=="par IDTRAJET")
    {
        choix=1;
    }
    ...
    trajet P;
            if(ui->tableView_2->currentIndex().row()==-1)
               ...
               else
               {
                           ...
                           P.setidtrajet(ui->tableView_2->model()->data(ui->tableView_2->model()->index(ui->tableView_2->currentIndex().row(),0)).toInt());
                 ...
                 QString  rawQr = "IDTRAJET:%1 DEPART:%2 DUREE:%3 TERMINUS:%4 PRIX:%5  " ;
                    rawQr = rawQr.arg(P.getidtrajet()).arg(P.getdepart()).arg(P.getduree()).arg(P.getterminus()).arg(P.getprix());
...
{
    Ticket T1;
    T1.setidt(ui->le_ID_2->text().toInt());
    bool test=T1.supprimer(T1.getidt());
    QMessageBox msgBox;
        if(test)
        { ui->tab_ticket_2->setModel(T.afficher());
...
void MainWindow::on_tabWidget_6_currentChanged(int index) //statistique
{
                    // background //
                     QLinearGradient gradient(0, 0, 0, 400);
                     ...
                     QCPBars *amande = new QCPBars(ui->plot_5->xAxis, ui->plot_5->yAxis);
                     amande->setAntialiased(false);
                     amande->setStackingGap(1);
                      //couleurs
                     ...
                     ui->plot_5->xAxis->setTickLabelRotation(0);
                     ui->plot_5->xAxis->setSubTicks(false);
                     ui->plot_5->xAxis->setTickLength(0, 4);
                     ui->plot_5->xAxis->setRange(0, 8);
                     ...
                     ui->plot_5->xAxis->setLabel("horaire d'entrée");

                     // axe des ordonnées
                     ui->plot_5->yAxis->setRange(0,10);
                     ui->plot_5->yAxis->setPadding(5);
                     ui->plot_5->yAxis->setLabel("Statistiques");
                     ...
                     ui->plot_5->legend->setBrush(QColor(250, 255, 255, 255));
...
void MainWindow::on_gestionstation_clicked()
{
    ui->stackedWidget->setCurrentIndex(1);
}


void MainWindow::on_gestionpersonnel_clicked()
{
    ui->stackedWidget->setCurrentIndex(2);
}


void MainWindow::on_gestionmetro_clicked()
{
    ui->stackedWidget->setCurrentIndex(3);
}


void MainWindow::on_gestiontrajet_clicked()
{
    ui->stackedWidget->setCurrentIndex(4);
}


void MainWindow::on_gestionticket_clicked()
{
